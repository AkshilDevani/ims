...
session_start();
  ...
  // $sql ="select inquiry.* , branch.branch_name , course.course_name , status.status_name , refernce.ref_name ,admin.name from `inquiry` join branch on inquiry.branch_id = branch.id JOIN course on inquiry.course = course.id JOIN status on status.id = inquiry.status JOIN refernce on refernce.id = inquiry.ref_id join admin on inquiry.inquiryby=admin.id";
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Inquiry Table</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-body">
                      ...
                      <tr>
                        ...
                        ?>
                          ...
                          <td>
                           ...
                           <th>
                            <a href="add_inquiry.php?id=<?php echo $data['id']; ?>">Edit</a> 
                            </th><th><a href="view_inquiry.php?id=<?php echo $data['id']; ?>">Delete</a>
                        </th>
                      </tr>
                    <?php  } ?>
                  </tbody>
                ...
                <div class="mt-3">
                  <a class="btn btn-primary btn-sm" href="view_inquiry.php?page_no=1">All</a>    
<?php
                  if($page_no>1)
                  {
                        echo "<a href='view_inquiry.php?page_no=".$page_no - 1 ."' class='btn btn-primary btn-sm' >pre</a>";

                  }
                  for ($i=1; $i<=$total_page; $i++)
                  {  
?>
                    <a class="btn btn-primary btn-sm" href="view_inquiry.php?page_no=<?php echo $i; if(isset($_GET['search'])) {?> &search=<?php  echo $_GET['search']; } ?>"><?php echo $i; ?></a>    
<?php     
                  } 
                  if($page_no<=$total_page-1)
                  {
                        echo "<a href='view_inquiry.php?page_no=".$page_no +1 ."' class='btn btn-primary btn-sm' >next</a>";
